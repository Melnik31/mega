...
export type SeasonGoal = Database["public"]["Tables"]["season_goals"]["Row"];
...
] as const satisfies readonly { key: keyof SeasonGoal; label: string }[];

export function MyYearView({ goal, editHref }: { goal: SeasonGoal; editHref?: string }) {
  return (
    <div className="flex flex-col gap-6">
      <div className="flex items-center justify-between">
        <h1 className="text-3xl">My Year</h1>
        {editHref && (
          <Link href={editHref} className="text-sm font-medium text-brand hover:underline">
            Edit
          </Link>
        )}
      </div>
            ...
            <div key={key} className="flex items-center gap-3 text-sm">
              ...
              <div className="h-2 flex-1 border border-black/10">
                <div
                  ...
                  style={{ width: `${((goal[key] as number) / 10) * 100}%` }}
                />
              </div>
